...
func generateEntityHelperMethods(sourceTypeName, validatorMethod, goPackagePath string, structType *types.Struct) error {
	...
	f.Line()
	generator.GenNew(f, sourceTypeName, publicFlds, validations, validatorMethod)
	generator.GenMustNew(f, sourceTypeName, publicFlds)

	f.Comment("Marshalers ...")
	f.Line()
	generator.GenUnmarshalFromStore(f, sourceTypeName, publicFlds, privateFlds)

	f.Comment("Accessors ...")
	f.Line()
	generator.GenGetters(f, sourceTypeName, genGetterFields)
	generator.GenSetters(f, sourceTypeName, genSetterFields)

	f.Comment("Utilities ...")
	f.Line()
	generator.GenEqual(f, sourceTypeName, equalFlds)
	generator.GenStringer(f, sourceTypeName, genStringerFields)
...
func getRelativeQualField(field *types.Var) generator.QualField {
	return generator.QualField{
		Id:      field.Name(),
		QualTyp: getQualifiedJenType(field.Type(), field.Pkg()),
	}
}

func getQualifiedJenType(ft types.Type, pkg *types.Package) *jen.Statement {
	switch t := ft.(type) {
	case *types.Basic:
		return jen.Id(t.Name())
	case *types.Array:
		return jen.Index().Add(getQualifiedJenType(t.Elem(), pkg))
	case *types.Slice:
		return jen.Index().Add(getQualifiedJenType(t.Elem(), pkg))
	case *types.Map:
		return jen.Map(getQualifiedJenType(t.Key(), pkg)).Add(getQualifiedJenType(t.Elem(), pkg))
	case *types.Named:
		if pkg == t.Obj().Pkg() {
